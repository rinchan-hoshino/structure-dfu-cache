...
- Watermelon Field dev.52 clone with Paxi and the 72.94 MB generated datapack removed:
  ...
  - one-resource override: exactly one new conversion, then warm-hit activation on the same reload.

## Boundaries

- Private prototype only: no public repository, Modrinth project, release, or pack adoption is authorized by this repository state.
- Initial cold construction is server-startup work. Subsequent resource reloads prepare a replacement snapshot asynchronously and activate it only at the reload barrier.
- The prototype does not yet garbage-collect unreferenced historical blobs or claim compatibility outside Minecraft 1.21.1 / NeoForge 21.1.248.
